...
open Sweep.UserRequestBodyModel
...
module UserApiHandlerTests =
  ...
  let ``CreateUser - Create user returns 0 where successful operation`` () =
    task {
      ...
      // or pass a body of type UserRequestBody
      let body = obj() :?> UserRequestBody |> Newtonsoft.Json.JsonConvert.SerializeObject |> Encoding.UTF8.GetBytes |> MemoryStream |> StreamContent
