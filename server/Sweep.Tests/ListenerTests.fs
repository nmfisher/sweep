...
module ListenerTests =
  ...
  let ``Validate Listener Condition`` () =
    task {
      let parse1 = Listener.parse (Some("AND FOO_EVENT WITHIN 12 DAYS MATCH ON BAZ"))  
      ...
      let parse2 = Listener.parse (Some("AND FOO_EVENT WITHIN 999 MINUTES MATCH ON BAZ"))  
      ...
      let parse3 = Listener.parse (Some("AND FOO_EVENT WITHIN 29 HOURS MATCH ON BAZ"))  
      ...
      let parse4 = Listener.parse (Some("AND FOO_EVENT WITHIN 29 HOURS MATCH ON NULL"))  
      ...
      Assert.Equal(None, Listener.parse None)

      Assert.Throws<Exception>(Action(fun () -> 
        Listener.parse (Some("AND FOO_EVENT WITHIN 999 DAYS MATCH ON BAZ"))  |> ignore
      )) |> ignore

      Assert.Throws<Exception>(Action(fun () -> 
        Listener.parse (Some("BLAH FOO_EVENT WITHIN 999 DAYS MATCH ON BAZ"))  |> ignore
      )) |> ignore

      Assert.Throws<Exception>(Action(fun () -> 
        Listener.parse (Some("AND FOO_EVENT INSIDE 999 DAYS MATCH ON BAZ"))  |> ignore
      )) |> ignore

      Assert.Throws<Exception>(Action(fun () -> 
        Listener.parse (Some("AND FOO_EVENT INSIDE 999 DAYS MATCH ON MATCH"))  |> ignore
      )) |> ignore

      Assert.Throws<Exception>(Action(fun () -> 
        Listener.parse (Some("AND FOO_EVENT WITHIN 999 SECONDS MATCH ON BAZ"))  |> ignore
      )) |> ignore

      Assert.Throws<Exception>(Action(fun () -> 
        Listener.parse (Some(""))  |> ignore
      )) |> ignore
